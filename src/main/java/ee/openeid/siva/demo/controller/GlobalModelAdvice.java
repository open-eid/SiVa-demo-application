...
public class GlobalModelAdvice {
    ...
    private final BuildInfoFileLoader buildInfoFileLoader;

    @ModelAttribute
    public void addBuildInfo(Model model) {
        try {
            model.addAttribute("buildInfo", buildInfoFileLoader.loadBuildInfo());
        } catch (IOException e) {
            model.addAttribute("buildInfo", new BuildInfo());
        }
    }
}
